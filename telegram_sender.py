...
Examples:
  ...
  python telegram_sender.py --all
    ...
    # Send all alerts
        ...
        print()
        send_daily_signals(signal_type="sell", limit=15)
        print()
        send_signal_summary()
        print()
    ...
    # Default: daily buy + sell signals (real-time + backtest validated)
    else:
        print("💡 No option specified. Sending today's buy & sell signals (use --help for options)\n")
        send_daily_signals(signal_type="buy", limit=15)
        print()
        send_daily_signals(signal_type="sell", limit=15)
